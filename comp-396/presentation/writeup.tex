...
\begin{frame}
  ...
  \visible<3->{
    \begin{equation*}
      \infer{
        \Gamma \proves^{n+1} M\mathtt{.next} \hastype A
      }{
...
\begin{frame}
  \frametitle{We need to go back... to the future.}
  ...
  \visible<2->{
    \begin{equation*}
      \infer{
        ...
        \Gamma^\prime \goal S \proves^n q \synth_k T
        &
...
\begin{frame}
  ...
  \begin{align*}
    ...
    \visible<4->{
      ...
      \only<8->{
        \code{map} fs\mathtt{.tail.next}\; xs\mathtt{.tail.next}
      }
    }
  \end{align*}
  ...
  \visible<9->{
    \begin{align*}
      ...
      \code{map} fs\mathtt{.tail}\; xs\mathtt{.tail}
    \end{align*}
  }
\end{frame}
...
\begin{frame}
  ...
  \pause
  (Time is codata. Same thing really.) \\
  \pause
  \begin{itemize}
    \item Finish implementation of a prototype language.
    \item Prove soundness/completeness w.r.t. the POPL'14 system.
  \end{itemize}
  \pause
